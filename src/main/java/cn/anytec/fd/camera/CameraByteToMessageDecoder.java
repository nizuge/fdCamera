...
//	private static final Logger logger = LoggerFactory.getLogger(CameraByteToMessageDecoder.class);
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		for (int i = 0; i < FDCameraData.MAX_FACE_ITEM_PER_FRAME; i++) {
			in.readBytes(data);
			//int confidence = byte2int(pojo);

			in.readBytes(data);
			//int ID = byte2int(pojo);

			in.readBytes(data);
			//double left = byte2int(pojo) / 640 ;

			in.readBytes(data);
			//double right = byte2int(pojo) / 640 ;

			in.readBytes(data);
			//double top = byte2int(pojo) / 360 ;

			in.readBytes(data);
			//double bottom = byte2int(pojo) / 360 ;
